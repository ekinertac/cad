"""
features/web/display.py — picker-row formatter for ``cad web``.

Shows repo, date, and title in fixed columns. Tiny module, but lives
on its own so the web feature's public surface ($init__) doesn't
have to crowd four shapes into one file.
"""
